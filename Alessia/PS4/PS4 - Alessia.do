...
* Overall, looking at the three graphs, it seems that pre-treatment are parallel in all three groups of villages. A small exception is represented by villages in the second group, which are being treated in 1996. In that category, we see that treated units show an increase in the probability of having a health centre in the village before the treatment, with respect to the control group. 
...
*This specification allows us to estimate the differential treatment intensity (depending on the number of INPRES schools constructed in the village), rather than estimating simply the treatment effect of having at least 1 INPRES school being constructed with the village (measured) with a dummy variable. That is, the coefficient on thw POST_INTENSITY variable represents the MARGINAL effect of constructing an extra school. Moreover, despite controlling for the POST dummy, since our Treatment variable in this case is not a binary variable, we are not estimating a saturated model, i.e., a model which has as many coefficients as the potential outcomes that the dependent variable can take. On the other hand, this is still a TWFE model, with village and year FE. Finally, the marginal effect of constructing an extra school according to the estimated model is 1.2 p.p. 
...
*Most of the unbalanced baseline controls included have statistically insignificant effects, except for the few of them that are significant at 5% and have a positive effect. Moreover, the treatment effect estimated by looking at the coefficient of POST_INTENSITY is almost unchanged with respect to the specification run in point (e). Therefore we can claim that our results remain robust to the inclusion of unbalanced baseline controls. 

*Adding the lagged outcomes in (g), we notice that the coefficient of the treatment effect, on the variable POST_TREATED decreases significantly with respect to the specification without the lag. This is likley due to the Nickel Bias (Nickel, 1981), explained in question (g): the bias is due to the correlation between the error term and the lagged dependent outcome. Moreover, if the the error terms are serially correlated across time we might not be able to retrieve a consistent estimator of the treatment effect. So, we can claim that the results are not robust to the inclusion of the lagged outcome. 
...
*(j)
* In a staggered DD design, such as the one implemented in Martinez-Bravo (2017), a potential obstacle to identification of the ATT are heterogenous treatment effects. 
*The Bacon decomposition theorem states that the two-way fixed effect (TWFE) estimator is a weighted average of all possible two-by-two DD estimates. Weights depend on the size and group variance in treatment and sum up to one.  Weights can also be negative - as discussed by de Chaisemartin and D'haultfoeuille (2020) - and in such case TWFE estimators may be biased even if the parallel trend assumption holds. As stated previously, the TWFE estimator is equal to the expectation of a weighted sum of the ATTs in all (g-t) cells, which compare the evolution of the outcome between treated and control groups at each point in time t. Negative weights arise because it may occur that the control group in some of the two-by-two DD may be treated in both periods, e.g., cases in which the late adopters (Treated group) are compared to early adopters (control group). Moreover, the prevelence of negative weights can be particularly problematic in the case in which early adopters units are more than late adopters - i.e., when the number of units treated decrease with time. 
*Negative weights when coupled with treatment effects heterogeneity across groups and times are an issue for the identification of the ATT. For example, if many ATTs in (g-t) cells are weighted by negative weights, the TWFE estimate will be biased towards zero or even take negative values, preventing its correct interpretation as the ATT. 
...
*Martinez-Bravo (2017) find evidence of heterogenous treatment effects, hence for the reasons states previously we cannot state a priori whether the TWFE estimates can be interpreted as average treatment effect. To do so, we must carry out further tests to check the robustness of the estimates to heterogenous effects. 
...
*However, since in our case the output of the command shows that, out of 15583 ATTs, all receive positive weights and none of them receive negative weight, we are fairly confident that the estimates obtained are robust to Treatment Effect Heterogeneity (de Chaisemartin & D'Haultfoeuille (2020a), page 2974), and so that the actual ATT and the the beta obtained from the TWFE estimation are not of different signs. In addition, as the Stata manual shows, since we are only getting one summary estimate, this further confirms that none of the weights are negative. Moreover, looking at the value of the standard error we observe as a result of both the command computation and the manual computation, 0.06796259, we can say that even if we had found some negative weights, the standard error magnitude is  large enough for us not to doubt that the estimated coefficient had flipped sign with respect to the actual "ATT".
